...
const METHODS = [
  ...
  "FastRW + paper fusion, no-self + direct target",
  "FastRW + paper fusion + prior anchor, no-self constraints",
  "FastRW + paper fusion + prior anchor, no-self + direct target",
...
function paperFusionWithDirectTarget(run, anchorCenter, includeSelf = false) {
  const M = run.data.M;
  const precision = Array.from({ length: M }, () => Array(M).fill(0));
  const rhs = Array(M).fill(0);
  const meanVar = meanVariances(run);
  ...
  for (let k = 0; k < run.data.K; k++) {
    ...
    if (!includeSelf && i === j) continue;
    const alpha = run.data.alpha_k[k];
    const pseudoObservation = run.data.b_k[k] + alpha * run.direct[j];
    const tau2 = Math.max(run.sampleVar[i] + alpha * alpha * meanVar[j], EPS);
    const w = 1 / tau2;
    precision[i][i] += w;
    rhs[i] += w * pseudoObservation;
    usedConstraints++;
  }
...
function writeCaseChart(caseSpec, rows) {
  ...
  const colors = {
    ...
    "FastRW + paper fusion, no-self + direct target": "#e377c2",
    "FastRW + paper fusion + prior anchor, no-self constraints": "#9467bd",
    "FastRW + paper fusion + prior anchor, no-self + direct target": "#7f7f7f",
...
function main() {
  ...
  for (const caseSpec of CASES) {
    ...
    for (const prior of priors) {
      ...
      const paperDirectTarget = paperFusionWithDirectTarget(fastrw, fastrw.direct, false);
      summaryRows.push(metrics(caseSpec, prior, "FastRW + paper fusion, no-self + direct target", paperDirectTarget.theta, fastrw.gt, paperDirectTarget.variances, {
        used_constraints: paperDirectTarget.usedConstraints,
        self_constraints: paperDirectTarget.selfConstraints,
      }));
      ...
      const priorAnchorDirectTarget = paperFusionWithDirectTarget(fastrw, priorValues, false);
      summaryRows.push(metrics(caseSpec, prior, "FastRW + paper fusion + prior anchor, no-self + direct target", priorAnchorDirectTarget.theta, fastrw.gt, priorAnchorDirectTarget.variances, {
        used_constraints: priorAnchorDirectTarget.usedConstraints,
        self_constraints: priorAnchorDirectTarget.selfConstraints,
      }));
